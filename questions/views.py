...
from .forms import QuestionSetForm, QuestionResponseFormDyno, QuestionForm #QuestionReturnForm
...
class WorksheetView(LoginRequiredMixin, CsrfExemptMixin, AccessRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        WorksheetForm = formset_factory(form=QuestionForm, formset=QuestionSetForm)        
...
class QuestionSetView(LoginRequiredMixin, CsrfExemptMixin, AccessRequiredMixin, DetailView):
    ...
    def post(self, request, *args, **kwargs):
        f = QuestionResponseFormDyno(request.POST)
        worksheet = QuestionSet.objects.get(pk=self.get_object().id)
        
        if len(request.POST)-1 < QuestionSet.objects.questions(id=worksheet.id).count():
            messages.error(request, 'Answer all.')
            self.object = None
            return render(request, self.template_name, self.get_context_data())
            # return render(self.request, self.template_name, {'feedback': 'answer all questions'})

        for i in request.POST:            
            if i != 'csrfmiddlewaretoken':
                question = SimpleQuestion.objects.get( pk=i[i.find('_')+1:] )
                try:
                    resp = QuestionResponse.objects.filter(user=request.user).get(question__id=question.id)
